...
- [x] Skills — a mana pool (regenerates passively, grows per level) and three active abilities on a bottom-of-screen hotbar (keys 1/2/3): Power Strike and Fireball (bonus damage at their own range, on top of a basic attack), and Heal (restores own hp). Each has its own mana cost and cooldown, server-validated (`useSkill` message, catalog in shared/skills.ts).
